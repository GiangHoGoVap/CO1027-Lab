...
Champion::Champion(int _id, int _health, int _damage) {
	...
	this->health = _health;
	this->damage = _damage;
	Champion::numberOfAliveChampions++;
	// EndTodo
}
...
bool Champion::attack(Champion& _victim) {
	//BeginTodo
	return ((_victim.health = _victim.health - this->damage) <= 0);
	//EndTodo
}
...
int fight(Champion*& a, Champion*& b, bool* eventList, int numberOfEvent) {
	...
	for (int i = 0; i < numberOfEvent; i++) {
		if (eventList[i]) { // a attack b
			if (a->attack(*b)) { // a killed b
				delete b;
				b = NULL;
				return 1;
			}
	...
	return 0; // No-one die after combat
	//EndTodo
}
